...
import { AlertTriangle, Building2, CloudSun, LocateFixed, Navigation, Route, Search, SunMedium, Trees, Waves, Wind } from "lucide-react";
...
export function App() {
  ...
  const searchSuggestions = useMemo(() => {
    ...
    // フォーカス時はすべての候補を表示（入力フィルタリングなし）
    const candidates = [...shelters, ...restSpots];
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[480px] flex-col bg-mist shadow-xl">
        <header className="space-y-3 border-b border-emerald-100 bg-white px-4 py-4">
          ...
          <form className="flex gap-2" onSubmit={handleStationSearch}>
            ...
            <button className="min-h-11 rounded-md bg-leaf px-4 text-sm font-semibold text-white">設定</button>
          </form>
          ...
          <form className="relative flex gap-2" onSubmit={handleSearchSubmit}>
            ...
            {showSearchSuggestions && searchSuggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-1 z-10 rounded-md border border-slate-200 bg-white shadow-lg max-h-60 overflow-y-auto">
